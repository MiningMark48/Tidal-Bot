...
class Music(commands.Cog):
    ...
    async def start_nodes(self):
        ...
        node = await self.bot.wavelink.initiate_node(host='127.0.0.1', port=2333, rest_uri='http://127.0.0.1:2333', password='hvQe9As3VGGS', identifier='Tidal', region='us_east')
    ...
    async def pause(self, ctx):
        ...
        await ctx.send('Pausing the track!')
        await player.set_pause(True)

    @commands.command()
    @commands.guild_only()
    async def play(self, ctx, *, query: str):
        """Play a track from YouTube, SoundCloud, a Twitch/Mixer stream, Vimeo, and more."""
    ...
    async def queue(self, ctx):
        """View the current queue of tracks."""
        player = self.bot.wavelink.get_player(ctx.guild.id)
        controller = self.get_controller(ctx)
    ...
    async def shuffle(self, ctx):
        ...
        if len(controller.queue._queue) < 3:
            return await ctx.send('Please add more tracks to the queue before trying to shuffle.')
    ...
    async def skip(self, ctx):
        ...
        await ctx.send('Skipping the track!')
        await player.stop()

    @commands.command(aliases=['disconnect', 'dc'])
    @commands.guild_only()
    async def stop(self, ctx):
        """Stop the currently playing track and disconnect the bot from voice"""
        player = self.bot.wavelink.get_player(ctx.guild.id)
        ...
        try:
            ...
            return await ctx.send('There was no track to stop, disconnecting anyway...')
