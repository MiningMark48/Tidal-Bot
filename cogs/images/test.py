...
class Images(commands.Cog):
    ...
    @staticmethod
    def processing(avatar_bytes: bytes, color: tuple) -> BytesIO:
        ...
        with Image.open(BytesIO(avatar_bytes)) as im:
            ...
            # background color being the user's color.
            with Image.new("RGB", im.size, color) as background:
    ...
    async def circle(self, ctx, *, member: discord.Member = None):
        """Display the user's avatar on their color."""
        ...
        async with ctx.typing():
            ...
            if isinstance(member, discord.Member):
                # get the user's color, pretty self explanatory
                member_color = member.color.to_rgb()
            else:
            ...
            fn = partial(self.processing, avatar_bytes, member_color)
